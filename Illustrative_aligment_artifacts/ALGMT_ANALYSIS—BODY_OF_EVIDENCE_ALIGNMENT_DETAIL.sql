...
WITH FIRST_STAGE (CONSTITUENT_LEARNING_STD_ID, LEARNING_STANDARD_ID, SIH_COURSEPK_ID) AS (
	...
	COURSE_GRAPH_EDGES_EXTENDED (CONSTITUENT_LEARNING_STD_ID, LEARNING_STANDARD_ID, SIH_COURSEPK_ID) AS (
		SELECT * FROM SECOND_STAGE UNION
		SELECT * FROM EXTEND_UP UNION
		SELECT * FROM FIRST_STAGE),
	COURSE_GRAPH_EDGES_WITH_UNPACKED (CONSTITUENT_LEARNING_STD_ID, LEARNING_STANDARD_ID, SIH_COURSEPK_ID) AS (
		SELECT * FROM SECOND_STAGE UNION
		SELECT * FROM FIRST_STAGE ),
	COURSE_GRAPH_EDGES (CONSTITUENT_LEARNING_STD_ID, LEARNING_STANDARD_ID, SIH_COURSEPK_ID) AS (
		SELECT * FROM COURSE_GRAPH_EDGES_WITH_UNPACKED MINUS
		SELECT * FROM FIRST_STAGE ),
...
SELECT DISTINCT org.NAME AS CAMPUS, crs_subj.COURSE_SUBJECTS, crs_grade.COURSE_GRADE_LVL,
				...
				(CASE WHEN clsm.COURSE_LEARNING_STD_MAP_SID IS NULL THEN 'PRIOR_AY_LEARNING_STD'
				...
				(CASE WHEN verts.LEARNING_STANDARD_ID IS NOT NULL THEN 'Aligned Standard'
					...
					WHEN ext_down.PROG_EXTENSION_DOWNWARD IS NOT NULL THEN 'Downward-Extensible'
																		ELSE 'Unaligned Standard' END) AS STD_PROGRESSION_ALIGNMENT,
				rec_meas.MEASUREABLE_STANDARD_YN, curric_span.SIH_COURSEPK_ID, sec.COURSE_SECTION_SID, curric_span.SIH_PERSONPK_ID, 
				rec_meas.LEARNING_STANDARD_ID, 
				CAST(CAST(rec_meas.LAST_EOL_UPDATE AS DATE) AS VARCHAR)  AS DATE_MEASUREMENT_LOADED,
				rec_meas.SIH_PERSONPK_ID || '_' || rec_meas.LEARNING_STANDARD_ID || '_' || rec_meas.EVIDENCE_OF_LEARNING_SID AS EVID_TUPLE
...
LEFT JOIN (SELECT DISTINCT clsm.SIH_COURSEPK_ID, cont.SUBJECT_TITLE, cont.GRADE_LEVEL, clsm.COURSE_LEARNING_STD_MAP_SID
			FROM IBMSIH.COURSE_LEARNING_STD_MAP clsm
...
LEFT JOIN COURSE_GRAPH_VERTICES verts ON verts.SIH_COURSEPK_ID = curric_span.SIH_COURSEPK_ID
...
ORDER BY org.NAME, crs_subj.COURSE_SUBJECTS, crs_grade.COURSE_GRADE_LVL, course.COURSE_ID,
		...
		CAST(CAST(rec_meas.LAST_EOL_UPDATE AS DATE) AS VARCHAR) DESC
